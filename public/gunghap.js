...
(function() {
  ...
  function saveMyData(){if(!window._lastSaju||!window._lastMBTI)return;var s=window._lastSaju;addPerson({id:'me',name:'나',ilju:s.P[2].s+s.P[2].b,mbti:window._lastMBTI,gender:(typeof ST!=='undefined')?ST.gender:'',birthInfo:(typeof ST!=='undefined')?{y:ST.y,m:ST.m,d:ST.d,h:ST.h||'',min:ST.min||''}:{},hasFull:true,saju:s,dw:window._lastDW,gg:window._lastGG,mbtiObj:window._lastMBTIObj,aiResult:window._lastAIResult||null,savedAt:Date.now()});try{var _h=JSON.parse(localStorage.getItem('mbts_history')||'[]');for(var _i=0;_i<_h.length;_i++){if(_h[_i].isMyProfile){if(window._lastDW)_h[_i].dw=window._lastDW;if(window._lastGG)_h[_i].gg=window._lastGG;if(window._lastMBTIObj)_h[_i].mbtiObj=window._lastMBTIObj;_h[_i].mbti=window._lastMBTI;_h[_i].saju=s;break;}}localStorage.setItem('mbts_history',JSON.stringify(_h));}catch(_e){console.warn('[MBTS] saveMyData history sync fail:',_e);}renderPeopleList();}
