...
// AI 응답 텍스트 세탁 — 한자 및 음양비율 표현 제거
// engine.js READ-ONLY 대응: DB 저장 전 raw text 단계에서 처리.
// JSON string value 안에만 존재하는 패턴이라 raw regex가 구조를 깨뜨리지 않음.
function sanitizeForOutput(text) {
  if (!text) return text;
  text = text.replace(/[一-鿿]/g, '');
  text = text.replace(/\(?\s*음\s*\d+\s*[·:,]\s*양\s*\d+\s*\)?/g, '');
